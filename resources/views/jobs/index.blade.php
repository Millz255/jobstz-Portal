...
<head>
    ...
    <style>
        ...
        .job-listing h3 i.fa-clock { /* Style the clock icon in job listing titles */
        font-size: 0.9em; /* Slightly smaller than the title */
        vertical-align: middle; /* Align with the middle of the text */
        margin-left: 5px; /* Add a little space to the left */
        }
        .job-listing p {
            margin-bottom: 10px;
            color: #555;
                    ...
                    @foreach ($jobs as $job)
                        <div class="job-listing @if($job->expired) expired @elseif($job->soon_expiring) soon-expiring @endif" itemscope itemtype="http://schema.org/JobPosting"
                            @if($job->soon_expiring)
                                data-bs-toggle="tooltip" data-bs-placement="top" title="Job application deadline is approaching soon!"
                            @endif
                        >
